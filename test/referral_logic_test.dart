...
import 'package:go_app/features/auth/presentation/pages/registration_page.dart';
...
void main() {
  ...
  testWidgets('Referral Logic Verification', (WidgetTester tester) async {
    ...
    final referralRow = find.ancestor(of: find.text('Having a referral code'), matching: find.byType(Row));
    final checkboxFinder = find.descendant(of: referralRow, matching: find.byType(Checkbox));
    
    expect(checkboxFinder, findsOneWidget);
    expect(tester.widget<Checkbox>(checkboxFinder).value, isFalse); // Expect Unchecked
    ...
    final verifyBtnWidgetInitial = tester.widget<TextButton>(find.ancestor(of: verifyButtonFinder, matching: find.byType(TextButton)));
    expect(verifyBtnWidgetInitial.onPressed, isNull, reason: "Verify button should be disabled for empty code");
    ...
    await tester.pump();
    final verifyBtnWidgetShort = tester.widget<TextButton>(find.ancestor(of: verifyButtonFinder, matching: find.byType(TextButton)));
    expect(verifyBtnWidgetShort.onPressed, isNotNull, reason: "Verify button should be ENABLED for code length 3 (1-7 range)");
    ...
    final verifyBtnWidgetLower = tester.widget<TextButton>(find.ancestor(of: verifyButtonFinder, matching: find.byType(TextButton)));
    expect(verifyBtnWidgetLower.onPressed, isNotNull, reason: "Verify button should be ENABLED because input is auto-converted to UpperCase in BLoC state");

    // Case D: Valid Input -> "JC543KA"
    await tester.enterText(referralField, 'JC543KA');
    await tester.pump();
    final verifyBtnWidgetValid = tester.widget<TextButton>(find.ancestor(of: verifyButtonFinder, matching: find.byType(TextButton)));
    expect(verifyBtnWidgetValid.onPressed, isNotNull, reason: "Verify button should be enabled for valid code");
    ...
    final verifyBtnWidgetLong = tester.widget<TextButton>(find.ancestor(of: verifyButtonFinder, matching: find.byType(TextButton)));
    expect(verifyBtnWidgetLong.onPressed, isNotNull, reason: "Verify button should be enabled (BLoC truncates to 7 chars)");
    ...
    expect(find.text('Verified'), findsAtLeastNWidgets(2)); // Button text + Popup text
  });
}
